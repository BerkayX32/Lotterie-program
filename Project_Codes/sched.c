...
//2. PART
		//as long as a process waits the priority of that process
		//increases. (the ticket number)
    list_for_each(tmp, &runqueue_head) {
  		p = list_entry(tmp, struct task_struct, run_list);
      if((jiffies-p->prevJiffies) >12 && p->ticket<20){
        p->ticket++;
      }
      else if((jiffies-p->prevJiffies) <4 && p->ticket>1){
        p->ticket--;
      }
    }
//3. PART
		//guarantee that the randticket is going to be negative at the end
		//that means we select that process
		int sumoftickets=0;

		list_for_each(tmp, &runqueue_head) {
			int index1=0;
			p = list_entry(tmp, struct task_struct, run_list);
			//check if the user is chosen before
			if(p->ischosen==0){
				sumoftickets=sumoftickets+p->ticket;
			}
		}
//4. PART
		//random ticket
		int randticket;
		get_random_bytes(&randticket,sizeof(randticket));
		if(randticket<0){
			randticket=randticket*(-1);
		}
		if(randticket>=sumoftickets){
			randticket=(randticket%sumoftickets)+1;
		}
//5. PART
    int userid1=0;
		...
		list_for_each(tmp, &runqueue_head) {
			...
			if(p->ischosen==0){
				if(can_schedule(p,this_cpu)){
					randticket=randticket-(p->ticket);
					if(randticket<=0){
						next=p;
						p->ischosen=1;
						userid1=p->user->uid;
...
//6. PART
    list_for_each(tmp, &runqueue_head){
      p=list_entry(tmp,struct task_struct, run_list);
      if(p->user->uid==userid1){
        p->ischosen=1;
      }
    }
  }
	//lottery scheduler ends
  else{
  	list_for_each(tmp, &runqueue_head) {
  		p = list_entry(tmp, struct task_struct, run_list);
  		if (can_schedule(p, this_cpu)) {
